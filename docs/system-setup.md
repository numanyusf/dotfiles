...
**Currently added to `/etc/pam.d/sudo` only.** `/etc/pam.d/gdm-password` was
restored from `.pre-u2f.bak` (no `pam_u2f` line) — see §3 above for why.

**Validate without logging out:** for sudo, `unset SUDO_ASKPASS; sudo -k;
sudo id` and tap. For gdm-password (if re-testing tap-to-login):
`pamtester gdm-password numan authenticate` (tap → "successfully
authenticated").

- **If re-enabling tap-to-login in the future:** re-add the line to
  `gdm-password`, then also fix the keyring — either accept the "unlock
  keyring" popup, or set the GNOME Login keyring's password to blank
  (Passwords and Keys app → Login → Change Password → leave new password
  blank) so it auto-unlocks without the PAM password. Blank is a reasonable
