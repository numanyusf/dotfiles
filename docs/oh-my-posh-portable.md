...
A standalone handout for replicating **just this oh-my-posh prompt**
...
**Universal manual install (any distro):**
```bash
...
- This is **only the prompt.** The `ls` colors (vivid `LS_COLORS`) and eza icon
  aliases from this dotfiles repo are separate — grab `ls_colors` and the eza aliases
